...
### 1) import calibration and measurement parameters. Note: names are case sensitive!
       probe.name <- "probe11_75mm"    ## ## choose probe (it must match one of the existing probe.ID in tdr.probe.par.txt)
       mux.level <- "mux0"       ## ## choose the multiplexer level used for the measurements. Allowed options are: mux0, (mux1), mux2   
       tdr.name <- "Dan.PhD"     ## ## choose the TDR unit used for the measurements. Allowed options are: TDR19200 or Dan.PhD, Giulio.PhD, ATU.lab, ATU.red, ATU.blue
       win.Lapp <- 1           ## ## input chosen length parameter (this is important in case something different from calibration is used)  
...
#              if(Splits[length(Splits)] == "mat") { format.wave <- "Matlab" } 
      ...
      for(ii in 1:n.selected.waves) {       # select one or multiple measurements to analyse 
            ...
            if(extension == "DAT") {
              wave <- read.delim(path.name[ii,1], header=F)  #read.table("water.DAT", sep='\t', dec=',', header=F)
            ...
            if(extension == "dat") {
              wave <- read.delim(path.name[ii,1], sep=',', header=F, skip=4)  #read.table("water.DAT", sep='\t', dec=',', header=F)
...
## ## Ka <- NA}                 # I removed <<- 
                    ...
                    #stop()
                       ...
                       output.Ka[ii,] <- c(file.name, Ka, vwc.topp, refpoint, endpoint) 
...
## convert output to dataframe, change the format of the columns and add temperature and factors contained in the filename:
      ...
      colnames(output.Ka) <- c("file.name", "Ka", "vwc.topp", "refpoint", "endpoint") 
