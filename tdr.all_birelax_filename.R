##tdr.all_birelax_filename <- function(...) {
# Giulio Curioni
# 17 August 2017

 # note: use this script ONLY IF THE FILES HAVE BEEN SAVED IN A STANDARD WAY (see examples below). Alternatively use tdr.all_birelax.R
 # IMPORTANT: because the filename does not contain information on which TDR unit was used, analyse only measurements taken with 
 # any probe and any multiplexers but with the same TDR unit. MANUALLY CHANGE THE TDR UNIT (see below). 
...
#check ### before running the script (note: search ### ### for important points for TDR inversion)
         ...
         options(windowsTimeout = c(100000, 100000)) # see help for windows {grDevices}, try using a longer delay when plotting MANY waveforms
         options(device="windows")
         #options(digits=3)

aa <- as.character(readline("\n\nONLY select the measurements taken with the same TDR unit (with any probe, mux0 or mux2) and modify tdr.name in the script.\nThis script assumes that the files were saved in a standard name format.\nPRESS ENTER TO CONTINUE"))  
if(aa != "") {stop("Error. You didn't press ENTER.")}
rm(aa)         
...
#  message("Run waves.to.table.R first!")
          ...
          #perm[,2:ncol(perm)] <- data.frame(lapply(perm[,2:ncol(perm)], as.numeric), stringsAsFactors=FALSE)     
          ...
          cond <- read_delim("waves.BEC.txt", delim = "\t", col_names = F,   
                 locale = locale(date_names = "en", date_format = "%Y-%m-%d", time_format = "%H:%M:%S", decimal_mark = ".", tz = "UTC"),
                 na = "NA", skip = 0, n_max = Inf)
          #cond[,2:ncol(cond)] <- data.frame(lapply(cond[,2:ncol(cond)], as.numeric), stringsAsFactors=FALSE) 
          cond <- data.frame(cond)
          colnames(cond) <- c("file.name",	1:(ncol(cond)-1))
          
  
          #define number of measurements to analyse
          n.waves <- nrow(perm)
      ...
      colnames(output.fft) <- c(
              ...
              ## split file names to extract probe name and mux [important! this assumes that the files have been saved in a standard way, with the probe name before the mux level separated by - ]
              perm$file.name <- as.character(perm$file.name)
              file_name <- perm$file.name                   
            
                            # general split
                            Splits <- unlist(strsplit(file_name[which.max(nchar(file_name))], "-"))  # it assumes that each waveform is named in a standard way
                            mux.col <- which(Splits == "mux0" | Splits == "mux1" | Splits == "mux2")
                            probe.col <- mux.col-1   # note: this assumes that the probe name is before the mux level
                            start.col <- mux.col+1   # note this assumes that the start is just after the mux level
                            length.col <- mux.col+2  # note this assumes that the length (win.Lapp) is after the start 
                            split.name <- matrix(nrow=nrow(perm), ncol=length(Splits))
                            split.name <- data.frame(split.name)
                            for (ll in 1:nrow(perm)) {
                                #split.name[ll,] <- unlist(strsplit(file.name.col[ll], "[.]"))
                                ll.split <- unlist(strsplit(file_name[ll], "-"))
                                split.name[ll,1:length(ll.split)] <- ll.split
                                }  
                                 
               extracol <- matrix(nrow=n.waves, ncol=ncol(split.name))
               extracol <- data.frame(extracol)
               colnames(extracol) <- c("probe.name", "mux", "repetition", "sample_rep")
...
### 1) import calibration and measurement parameters. Note: names are case sensitive!
       #probe.name <- "lab1.3"    ## ## choose probe (it must match one of the existing probe.ID in tdr.probe.par.txt)
       #mux.level <- "mux2"       ## ## choose the multiplexer level used for the measurements. Allowed options are: mux0, (mux1), mux2   
       probe.name <- split.name[ii, probe.col]
       mux.level <- split.name[ii, mux.col]
       tdr.name <- "ATU.lab"     ### ### IMPORTANT: choose the TDR unit used for the measurements. Allowed options are: TDR19200 or Dan.PhD, Giulio.PhD, ATU.lab, ATU.red, ATU.blue 
       print(probe.name)
...
## define probe parameters corresponding to the chosen probe (see probe.name above)
    ...
    ### ### IMPORTANT: this currently works only for 15cm probes!
...
### ### if using custom parameters (e.g. window length for TDR inversion) manually modify and uncomment the parameters (uncomment only the ones to be modified):
    # note: the start.plot value does not make a difference in the calculation of permittivity so it is not added here
         ...
         win.Lapp <- as.numeric(substr(split.name[ii,length.col], start=0, stop=nchar(split.name[ii,length.col])-1))
         start.plot <-as.numeric(substr(split.name[ii,start.col], start=0, stop=nchar(split.name[ii,start.col])-1))
...
#                       write.table(output.BEC, paste(probe.name, ".BEC.txt", sep=''), sep='\t', row.names=F, col.names=c("file.name.bec", "BEC.mS.m", "RC"), quote=F, append=T)
                       write_delim(output.Ka, "multiple.output.Ka.txt", delim='\t', append = T, col_names=T)
                       write_delim(output.tdr, "multiple.output.tdr.txt", delim='\t', append = T, col_names=T)
                       write_delim(output.BEC, "multiple.output.BEC.txt", delim='\t', append = T, col_names=T)
...
if(inversion == TRUE) {
                      ...
                      output.fft <- cbind(output.tdr, output.fft[,-1])
                       ...
                       write_delim(output.fft, "multiple.output.fft.txt", delim='\t', append = T, col_names=T)                       
